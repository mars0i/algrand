...
(defn convert-number
  ...
  internally.  If the fourth argument exists and is not nil or false, uses
  doubles instead."
  [x base digits & [use-double]]
  (let [x' (if use-double (double x) (rationalize x))
        int-part (bigint (m/floor x')) ; coerce away ".0", might as well use BigInt rather than Integer
        fract-part (- x' int-part)]
    (println x x' int-part fract-part)
    (apply str 
           (concat
